...
// O(nlogn) due to the array sort, otherwise O(n)
func part1(input []int, targetSum int) {
	sort.Ints(input)
	l := 0
	r := len(input) - 1
	for l < r {
		if input[l]+input[r] < targetSum {
			l++
		} else if input[l]+input[r] > targetSum {
			r--
		} else if input[l]+input[r] == targetSum {
			break
		}
	...
	fmt.Printf("Numbers: %d, %d. Product: %d\n", input[l], input[r], product)
}

// O(n^2logn)
func part2(input []int, targetSum int) {
	sort.Ints(input)
	l := 0
	m := l + 1
	r := len(input) - 1
	...
	for ; l < m; l++ {
		...
		for m < r && !found {
			if input[l]+input[m]+input[r] < targetSum {
				m++
			} else if input[l]+input[m]+input[r] > targetSum {
				r--
			} else if input[l]+input[m]+input[r] == targetSum {
				found = true
			}

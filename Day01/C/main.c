...
void part1(int input[], int size, int targetSum)
{
    int l = 0;
    int r = size - 1;
    while (l < r) {
    ...
    int product = input[l] * input[r];
    printf("Numbers: %d, %d. Product: %d\n", input[l], input[r], product);
}

// O(n^2logn) time complexity
// O(n^2logn) because of the sort (O(nlogn)) running n times worst case
void part2(int input[], int size, int targetSum)
{
    int l = 0;
    int m = l + 1;
    int r = size - 1;
